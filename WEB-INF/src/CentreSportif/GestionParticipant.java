...
public class GestionParticipant {
    ...
    public GestionParticipant(TableParticipants participants, TableEquipes equipes, TableLigues ligues) throws IFT287Exception {
        ...
        if (participants.getConnexion() != equipes.getConnexion())
            throw new IFT287Exception("Les instances de TableParticipants et de TableEquipes n'utilisent pas la mÍme connexion au serveur");
    ...
    /**
     * Ajout d'un nouveau participant dans la base de donn√©es. S'il existe d√©j√†, une
     * exception est lev√©e.
     */
    public void inscrireParticipant(String prenom, String nom, String motDePasse, int matricule)
            throws SQLException, IFT287Exception, Exception {
        try {
            // V√©rifie si le particpant existe d√©ja
            if (participants.existe(matricule))
                throw new IFT287Exception("Participant existe dÈj‡†: " + matricule);
    ...
    /**
     * Suppression d'un partcipant de la base de donn√©es.
     */
    public void supprimerParticipant(int matricule) throws SQLException, IFT287Exception, Exception {
        try {
            // V√©rifie si le participant existe et qu'il fait partie d'une equipe
            ...
            if (tupleParticipant.getNomEquipe() != null)
                throw new IFT287Exception("Le participant avec la matricule: " + matricule + " fait partie de l'Èquipe " + tupleParticipant.getNomEquipe());
    ...
    /**
     * Permettre ‡ un particpant de s'inscrire dans une Èquipe.
     */
    public void ajouterJoueur(String nomEquipe, int matricule ) throws SQLException, IFT287Exception, Exception {
        try {
            ...
            if (tupleEquipe == null)
                throw new IFT287Exception("Nom d'Èquipe inexistant: " + nomEquipe);
            TupleParticipant tupleParticipant = participants.getParticipant(matricule);
            if (tupleParticipant == null)
                throw new IFT287Exception("Participant inexistant: " + matricule);
    ...
    public void supprimerJoueur(String nomEquipe, int matricule ) throws SQLException, IFT287Exception, Exception {
        try {
            ...
            if (tupleParticipant.getNomEquipe()== null )
                throw new IFT287Exception("Le participant avec la matricule: " + matricule + " n'a pas d'Èquipe");
            
            if (!tupleParticipant.getNomEquipe().equals(nomEquipe))
                throw new IFT287Exception("Le participant avec la matricule: " + matricule + " ne fait partie de l'Èquipe: " + nomEquipe);
    ...
    public void refuserJoueur(String nomEquipe, int matricule ) throws SQLException, IFT287Exception, Exception {
        try {
            ...
            if (tupleParticipant.getNomEquipe()== null )
                throw new IFT287Exception("Le participant avec la matricule: " + matricule + " ne fait partie d'une Èquipe");
            
            if (!tupleParticipant.getNomEquipe().equals(nomEquipe))
                throw new IFT287Exception("Le participant avec la matricule: " + matricule + " n'est pas inscrit ‡ l'Èquipe: " + nomEquipe);
            
            if (tupleParticipant.getMatricule() == tupleEquipe.getMatriculeCapitaine())
                throw new IFT287Exception("Le matricule est celui du capitaine: " + matricule);

            if(tupleParticipant.getEstAccepter() == 1)
                throw new IFT287Exception("Participant: " + matricule + " fait dÈj‡ partie de l'Èquipe. Veuillez supprimer le joueur de l'Èquipe." );
